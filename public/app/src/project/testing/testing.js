...
function TestingController($scope, $stateParams, SprintFactory, TaskFactory, BoardFactory, $mdSidenav) {
    ...
    function getData() {
        SprintFactory.testingSprint($scope.project_id).then(function(response) {
            if (response == null) {
                $scope.activeSprint = [];
            } else {
